...
* Version: 3.5 - Fixed WhatsApp message pre-fill issue
*/
...
document.addEventListener('DOMContentLoaded', function() {
 ...
 /**
  ...
  /**
   * Checkout via WhatsApp (Fixed version with proper message pre-fill)
   */
  function checkoutViaWhatsApp() {
    ...
    // Build message without special formatting that might break
    let message = 'New Order - AfriMart Depot\n\n';
    ...
    cartItems.forEach((item, index) => {
        ...
        message += `   Qty: ${item.quantity} x ${item.price} = $${itemTotal.toFixed(2)}\n`;
        if (item.variant) {
    ...
    const shippingCost = shippingEl ? shippingEl.textContent : '$5.99';
    message += `Shipping: ${shippingCost}\n`;
    
    // Get tax
    const taxEl = document.getElementById('tax-amount');
    const taxAmount = taxEl ? taxEl.textContent : '$' + (subtotal * 0.08).toFixed(2);
    message += `Tax: ${taxAmount}\n`;
    
    // Get total
    const orderTotalEl = document.getElementById('order-total');
    if (orderTotalEl && orderTotalEl.textContent) {
        message += `TOTAL: ${orderTotalEl.textContent}\n`;
    } else {
        const shipping = parseFloat(shippingCost.replace(/[^\d.-]/g, '')) || 5.99;
        const tax = parseFloat(taxAmount.replace(/[^\d.-]/g, '')) || (subtotal * 0.08);
        const finalTotal = subtotal + shipping + tax;
        message += `TOTAL: $${finalTotal.toFixed(2)}\n`;
    }
    
    message += '\nPlease provide your delivery address and preferred payment method.';
   ...
   // WhatsApp business number - use wa.me format which is more reliable
    const whatsappNumber = '18048060130'; // US Country Code 1 + Number (no '+')
    
    // Create WhatsApp URL using wa.me format (more reliable than api.whatsapp.com)
    let whatsappUrl = `https://wa.me/${whatsappNumber}?text=${encodedMessage}`;
    
    console.log('WhatsApp URL:', whatsappUrl);
    console.log('Encoded URL length:', whatsappUrl.length);
    
    // If URL is too long (over 2000 characters), shorten the message
    if (whatsappUrl.length > 2000) {
        console.warn('URL too long, creating shorter message...');
        
        let shortMessage = 'AfriMart Depot Order\n\n';
        cartItems.forEach((item, index) => {
            shortMessage += `${index + 1}. ${item.title} (x${item.quantity})\n`;
        });
        shortMessage += `\nTotal: ${orderTotalEl ? orderTotalEl.textContent : '$' + subtotal.toFixed(2)}`;
        shortMessage += '\n\nDelivery address?';
        
        const encodedShortMessage = encodeURIComponent(shortMessage);
        whatsappUrl = `https://wa.me/${whatsappNumber}?text=${encodedShortMessage}`;
    }
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(message).then(() => {
        ...
        showAddedToCartNotification('Opening WhatsApp... Order details also copied to clipboard!');
    }).catch(err => {
...
try {
    ...
    // Final fallback: show the message for manual copy
    alert('Could not open WhatsApp automatically. Your order details have been copied to clipboard. Please paste them in WhatsApp.');
}
